...
The public Python API for callers embedding Modulatio in their own code. v0.1.0's surface area is intentionally narrow — most users interact through the CLI / TUI, not Python — but the engine core is importable for users who need to embed.
...
The unit of work in v0.1.0. Created by the wizard or the `modulatio project create` CLI; lives at `<vault>/<code>/`.
