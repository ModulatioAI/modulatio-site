...
(A fourth entry point, `modulatio-api` — a FastAPI server for a planned web dashboard — is on the long-horizon roadmap but did NOT ship in v0.1.0. The console script is intentionally not declared until the underlying module exists. See [Roadmap](/roadmap/).)
...
**Engine calibration banner:** between the Sandbox section and the Alerts section, `doctor` surfaces what v0.1.0 is and isn't sized for. This sets correct expectations on first contact:
