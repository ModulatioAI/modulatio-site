...
description: Phase 1 of a 200-page novel — the honest scope for production-scale work in v0.1.0.
---

v0.1.0's honest contract: **production-scale work runs as Phase 1 only in this release**. A 200-page novel, a multi-feature application across many releases, a multi-quarter research program — the engine can ship Phase 1 of any of these. The remaining phases need the job-template + persona-continuity work landing in a later release to run cleanly across cross-phase boundaries.

This page walks through Phase 1 of a production-scale effort end-to-end. The headline is: v0.1.0 does Phase 1 well. Phase 2+ is human-driven across phase boundaries until job templates ship.
...
We're going to plan + execute Phase 1 of a 200-page novel. Plenty of canonical examples here — fiction-length work is the class job templates target most directly. For v0.1.0, we ship Phase 1 (the first ~5 chapters, ~40 pages) and stop; later phases go on the user's queue.
...
v0.1.0 can't carry that across phases on its own. So we cap to Phase 1 deliberately, ship a self-contained slice the user can review, and queue the rest for the next release.
...
Leader engages plan-mode. The leader-plan skill detects an artifact-shipping intent with a concrete size signal ("200-page novel") that exceeds v0.1.0's single-phase capacity. The deliverable-shape clarifying question fires:
...
Per the v0.1.0 contract for production-scale answers, Leader does NOT produce a plan covering the whole novel. It produces Phase 1 only:

```markdown
<!-- modulatio:plan -->

### Diagnostic
This is production-scale; this plan covers Phase 1 only. v0.1.0
...
- **Cross-phase tooling not yet shipped.** Phase 2+ depends on
  ...
  200-page goal won't complete inside v0.1.0.
...
When job templates land, this state document becomes the seed for the persistent cross-phase memory. For v0.1.0, it's the user's read-out at the end of Phase 1.
...
v0.1.0's safety nets fire harder on production-scale work:
...
Until then, v0.1.0's contract is honest: ship Phase 1, queue Phases 2+ for the next release. The work doesn't disappear; it queues.
...
- [v0.1.0 Beta calibration](/v0-1-0-beta/) — the engine's honest scoping for production-scale work.
