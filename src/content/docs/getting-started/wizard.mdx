...
- **Anthropic Claude CLI OAuth** — if `claude` is on PATH and authed, offers to wire it in via subprocess. Pro/Max attribution status is gray-area; see [Providers & models](/concepts/providers/#anthropic) for the trade-off.
- **OAuth tokens for Anthropic / OpenAI** found in `~/.config/anthropic/` or `~/.config/openai/`
...
- Auth method: `api_key`, `oauth_anthropic`, `oauth_openai`, `cli_subprocess`, or `none` (for unauthenticated local services)
