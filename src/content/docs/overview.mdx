...
**[v0.8.6 release notes](/v0-8-6/)** — what's new in the current release: Leader self-remediation (fix fixable concerns in place, under a typed gate + engine-owned fix window) and JT generativity (refuse a job template a job can't fill; derive a fitting one; cron skip-the-slot). The prior **[v0.8.4 release notes](/v0-8-4/)** cover deliverable fidelity: verify the whole assembled deliverable against the brief — the `DeliverableSpec`, an engine-extracted digest + readable twin that give the verifier eyes, a produce-time per-part floor, engine-generated framing (title + TOC), and cross-part numbering, all per-family dispatch. See the [Deliverable fidelity](/architecture/deliverable-fidelity/) deep-dive.
