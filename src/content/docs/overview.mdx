...
v0.5.0 gives every job its **own output folder** and makes recurring work codify into **Job Templates** — the Leader's own self-authored interview, parameter schema, and output contract for a *class* of job, bindable to a headless cron payload and improved by the team when it notices you keep running the same kind of job. It's the setup-side of the **Alfred loop**: where v0.4.0 made the team **learn from its own failures** (codifying recurring QC corrections into durable, **git-versioned** skill guidance that cheap producers load next time), v0.5.0 makes it learn from your repeated *setup*. It builds on v0.3.0's **skill-library keystone** (a producer is now a **model endpoint** that checks skills out of a shared library per task, so *any* producer runs *any* task, and dispatch routes on **capability + availability and never blocks** — work spreads across idle models, and a task that can't find its ideal producer runs on the best-available one with an honest **Product Quality Report** reservation instead of a dead end), v0.2.2's **web search** + **provably-terminating redo loop**, v0.2.1's **in-place editing**, and v0.2.0's **QC-thesis arc** (cheap producers generate the bulk; the smarter QC reviews and patches only the errors; reservations ship as an advisory Product Quality Report, never a blocker). Validated end-to-end on live heterogeneous model lineups and cleared fresh two-reviewer audits. Read the [v0.5.0 release notes](/v0-5-0/) and the [Beta calibration page](/v0-1-0-beta/) before kicking off serious work — knowing the engine's real ceilings is the difference between a smooth run and a frustrated one. **Python 3.12+**. Bug reports and discussions welcome at the [issues tab](https://github.com/ModulatioAI/modulatio/issues) and [discussions](https://github.com/ModulatioAI/modulatio/discussions).
</Aside>
...
**[v0.5.0 release notes](/v0-5-0/)** — what's new in the current release: per-job output folders + Job Templates — the setup-side of the Alfred loop. Each job gets its own folder, and recurring work codifies into reusable, git-versioned templates that bind to a headless cron payload.
