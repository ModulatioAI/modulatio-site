...
v0.8.2 **completes the assembler families and adds a cost-governed tool tier.** The assemblers are now whole — `document` (ordered text), `code` (preserve the file tree + a wiring index), `data` (a real JSON/CSV merge), and **`media`** (join image/audio/video/bundle units with a local compositor — stdlib zip, ffmpeg concat, ImageMagick — engine-owned, fails closed when a tool is absent). Every family keeps the same spine: the producer emits a small *plan* (a manifest), **the engine owns the join**, and unit bytes never round-trip through the model. This release also adds a general, **fail-closed metered-tool tier** (the free-DDG / metered-Tavily pattern): a tool can mark itself paid, and the Comptroller gates every metered call *before* it spends — deny on missing budget, per-task + daily caps, idempotent, narrow params, only ever on QC-passed pinned inputs. Free-local stays the default and the tier ships with no provider or key (a mechanism, not a vendor binding). It builds on [v0.8.1](/v0-8-1/)'s **content-addressed review-ledger** (QC verifies the *marks*, not the bytes), [v0.8.0](/v0-8-0/)'s **ACP server**, v0.7's **conversation-first Leader**, and the **QC-thesis arc** from v0.2.x (cheap producers generate the bulk; the smarter QC reviews and patches only the errors). Cleared fresh **hull + coherence** reviews (five hull holes found and sealed); full suite green on a renderer-free CI box. Read the [v0.8.2 release notes](/v0-8-2/) and the [Beta calibration page](/v0-1-0-beta/) before kicking off serious work — knowing the engine's real ceilings is the difference between a smooth run and a frustrated one. **Python 3.12+**. Bug reports and discussions welcome at the [issues tab](https://github.com/ModulatioAI/modulatio/issues) and [discussions](https://github.com/ModulatioAI/modulatio/discussions).
</Aside>
...
**[v0.8.2 release notes](/v0-8-2/)** — what's new in the current release: the fourth assembler family (media — join image/audio/video/bundle units with a local compositor) and a general, fail-closed metered-tool tier (paid tools gated before they spend; free-local stays the default).
