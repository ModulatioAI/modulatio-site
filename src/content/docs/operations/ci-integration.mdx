      ...
      - name: Run kickoff
        run: |
          .venv/bin/modulatio kickoff \
            --code CIVAL \
            --objective "review the docs in this PR for v0.1.0 alignment"
...
2. **Configure auto-approval** for specific ticket classes. (Not yet supported in v0.1.0; worth a feature request if you need it.)
...
(v0.1.0 may not have a CLI flag for this; the equivalent is constructing an `Orchestrator` programmatically with `team_memory_enabled=False, qc_history_embedder=None, semantic_matcher=None`.)
