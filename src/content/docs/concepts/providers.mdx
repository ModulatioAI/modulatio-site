...
As of v0.7.0 you can wire providers, models, keys, and agents from a menu-driven
...
built-in catalog of eleven providers; you type only a key. The same tab adds and
removes models and agents, and manages **multiple keys per provider** — including
the [key-pool](/architecture/key-pool/), which puts several of your own keys
behind one model for throughput and rate-limit failover. The CLI commands below
remain available and edit the same `model_presets.json`.
