...
- **`oauth_xai`** — xAI (Grok) OAuth via Modulatio's own sign-in: run `modulatio auth login-xai` with a **SuperGrok / X Premium+** subscription. Tokens live in Modulatio's own credentials file and auto-refresh — xAI rotates the refresh token on every refresh, so Modulatio persists each rotation. A 403 on refresh means the subscription tier isn't authorized for API access; use the `api_key` path instead.
...
<Aside type="tip" title="Some GGUFs need a tool template">
If you pull a GGUF that doesn't ship a tool-calling template, tool-using producers (research, run_shell) will fail silently. Either pull an alternative quant that includes the template, or set `supports_tools: false` on the model entry so dispatch never routes tool-loadout tasks to it.
</Aside>
