...
Two honest limits: the released call's thread is *abandoned*, not killed (CPython can't kill a thread) — it burns quietly until the transport lets go, and the log line counts live "zombies" so accumulation is visible; and a stall that holds the GIL inside a C extension freezes the whole process — nothing in-process can run, and the dump from any future wedge is exactly what would prove that shape.
