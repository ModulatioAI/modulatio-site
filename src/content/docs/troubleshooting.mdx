...
### "GGUF tool calls fail silently"

Some GGUFs ship without a tool-calling chat template, so tool-using producers (research, run_shell) appear to work but emit no tool calls.
