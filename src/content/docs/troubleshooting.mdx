...
Cause: v0.1.0's catch — a producer call's prompt + context exceeded the model's window even after Layer 2's compression pass. Re-running with the same prompt would hit the same wall.
...
2. **Decline if the work is genuinely too big for v0.1.0.** Production-scale efforts queue for a later release — see [Roadmap](/roadmap/).
...
### "plan has X tasks — exceeds the v0.1.0 hard cap of 6"
...
Cause: v0.1.0 is honest — multi-phase / long-running work is later-release territory.
