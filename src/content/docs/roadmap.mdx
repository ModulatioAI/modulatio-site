...
v0.9.4 is the current Beta release — **the two-lane Leader**: a standalone coding agent you can pair with directly, plus three autonomy modes. This page consolidates what's shipping next (the **1.0** line) and the long-horizon pillars under design. Sections are intentionally light on dates — Modulatio ships when it's ready, not on a calendar — and heavy on what each direction is *for*.
