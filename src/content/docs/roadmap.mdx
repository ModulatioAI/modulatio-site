...
v0.8.9 is the current Beta release — a **security hardening** release. This page consolidates what's shipping next (the **0.9.0** and **1.0** lines) and the long-horizon pillars under design. Sections are intentionally light on dates — Modulatio ships when it's ready, not on a calendar — and heavy on what each direction is *for*.
...
## Current — v0.8.9 (security hardening)

The current release is **v0.8.9 — security hardening** ([release notes](/v0-8-9/)). A full-codebase security audit of the agent engine, then **two independent mirror-audits** — an adversarial *hull* pass and a *coherence* pass, each a different model reviewing the whole tree fresh — to catch what one pass would miss. **Nine findings confirmed and closed**, the keystone (a tool-call authorization bypass that let a skill reach a tool outside its declared loadout) found by the *independent* pass, not the first. **No functional change for a normal run** — this is defense-in-depth on the surfaces a prompt-injected model could otherwise reach, every fix an engine-bound invariant rather than prompt guidance. The guiding rule: *a permission is a key to a door inside the ship; it never opens the sea valves.* See the [v0.8.9 release notes](/v0-8-9/) for the per-finding delta.

### Prior — v0.8.8 (deterministic assembly validation + codify-the-win)
...
## Next — v0.9.0 (stability + reporting)

The 0.9.0 thesis is **harden what's here, not grow the surface** — bug fixes and performance work, plus exactly one net-new feature: a built-in **bug + crash reporting tool**.

- **Bug + crash reporting to the repo.** When a run hits an unhandled error — or you hit a bug worth reporting — Modulatio packages the relevant context (redacted) into a structured report and files it straight to the project's [GitHub issue tracker](https://github.com/ModulatioAI/modulatio/issues), so a problem you hit becomes something the project can act on without you hand-writing a repro. The one new feature in this line.
- **Bug fixes.** The accumulated backlog from real runs — the rough edges that don't warrant a feature but do warrant a fix.
- **Performance improvements.** Throughput, context-budget efficiency, and routing — making the current capabilities faster and cheaper rather than adding new ones.

---

## Then — v1.0 (a new way to use it)
...
- **A new TUI.** The conversation-first overhaul — the Leader as a streaming partner you talk to (not an admin dashboard), agents addressed by the names you gave them, producer + QC activity streamed live.
