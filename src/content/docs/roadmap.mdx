...
## Current — v0.9.8.8 (live telemetry + a self-healing team)

The current release is **v0.9.8.8 — watch the run, and trust the team to finish it** ([release notes](/v0-9-8-8/)): the factory floor's **telemetry rail goes live** (task progress, QC tally, context load, and each producer's current move, in real time); the team **finishes the job even when a seat dies** — a crashed local model is recognized as an availability failure, its retries back off, the seat cools out of the dispatch pool, and **QC — the producer of last resort — authors the missing pieces** at a goal's end rather than shipping a hole; and the seats **tell the truth about themselves** when a reasoning model can't be quieted through its endpoint (flagged by `modulatio doctor`, the agent builder, and the run log). Proven by a live kill-test — a model killed mid-run, the team healed and shipped satisfied with zero human touches. Four cadre-cleared arcs.

Recent prior releases: **v0.9.8.7 — research reuse + a context-size fan** ([release notes](/v0-9-8-7/)), **v0.9.8.6 — single-source leadership** ([release notes](/v0-9-8-6/)), and **v0.9.8 — the Feng-Tui interface, finished** ([release notes](/v0-9-8/)).
