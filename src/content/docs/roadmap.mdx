...
The current release is **v0.9.9.5 — MCP servers + the Leader, unbound** ([release notes](/v0-9-9-5/)). Modulatio can now consume **MCP servers**: plug in a Model Context Protocol server (local `stdio` subprocess or remote `http` endpoint) with `modulatio mcp add-stdio`/`add-http` and its tools become first-class Modulatio tools — the Leader can call them (gated once/session/always, or run a read-only server trusted), a producer when a skill's loadout grants it — on the *same* control plane as the built-ins and the SERVICES pool (tool registry, write-only key vault, permission gate, spend comptroller). The **conversational Leader** now rides the model's **full context window** (a long thread isn't compressed away; cap it from SETTINGS for cost) and can read and change the team's own configuration directly. The **WebOS console** gains terminal-parity polish — the add-model **live picker**, **Reload services**, and copy-from-the-TVs that works like the terminal. Plus `modulatio auth login-xai` (SuperGrok / X Premium+ OAuth, Modulatio-owned) and a `doctor` model-call-stack probe. Every new surface cleared a two-lens security + quality adversarial review; 5,598 tests.
