...
- **A web-based UI.** *First cut shipped in [v0.9.9](/v0-9-9/) — the WebOS.* The Console and nine read-only pages are live; still ahead are the interactive CONFIG tab (models, agents, projects, settings, folders — read/write) and the interactive Console verbs (approve/deny a ticket, toggle a cron job, kick off a Job Template from a button, scoped approvals from the modal).
