...
v0.9.5 is the current Beta release — **subscription seats**: bring your own Claude and GPT-5.5 to the team (Clay runs any seat on your Claude Code subscription, GPT-5.5 on your OpenAI Codex subscription), plus per-seat fallback models. This page consolidates what's shipping next (the **1.0** line) and the long-horizon pillars under design. Sections are intentionally light on dates — Modulatio ships when it's ready, not on a calendar — and heavy on what each direction is *for*.
