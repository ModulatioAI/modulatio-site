...
## Current — v1.0.0b1 (the 1.0 line opens — public beta)

The current release is **v1.0.0b1** ([release notes](/v1-0-0b1/)) — the first release of the
**1.0 line**, shipped as a public beta. The Leader earns real hands and an honest console: a
solo coding harness hardened through eight adversarial security-review rounds (fix-in-place
remediation, sandbox-routed pytest evidence gating code goals, QC one-verdict-then-fix), a
conversational lane that cannot be blocked by anything except a permission ask, permission
grants that land on the very call that asked (Once/Session/Always all live), contained PDF
reading, and the WebOS console grown into a live mission board (Goal board + QC desk,
per-seat solo/mute filtering, markdown-rendered Leader replies). Being a pre-release, the
beta is opt-in: `pip install "modulatio[web]==1.0.0b1"` — a plain install stays on stable.
