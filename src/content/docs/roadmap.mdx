...
## Current — v0.9.8.9 (the hard kill-boundary)

The current release is **v0.9.8.9 — no stalled call outlives its clock** ([release notes](/v0-9-8-9/)): a model call that **spins or stalls in a way no network timeout can see** (the live 17-minute wedge) is now **force-released** 30s past its transport bound, fails as an availability error into the self-healing team (fallback, backoff, cooldown, QC backstop), and leaves an **all-threads stack dump** in the crash log. The tool-loop's transport bound now honors `MODULATIO_CALL_TIMEOUT` (default 600s) like every other call. Plus a **real title on ad-hoc assembled reports** and a splash-art fix. Both reviewers cleared the arc.

Recent prior releases: **v0.9.8.8 — live telemetry + a self-healing team** ([release notes](/v0-9-8-8/)), **v0.9.8.7 — research reuse + a context-size fan** ([release notes](/v0-9-8-7/)), and **v0.9.8.6 — single-source leadership** ([release notes](/v0-9-8-6/)).
