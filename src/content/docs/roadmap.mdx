...
v0.9.9.3 is the current Beta release — **the WebOS runs your calendar**: a real Teams-style scheduler, real run telemetry, and the engine's own permission gate in the browser. This page consolidates what's shipping next (the **1.0** line) and the long-horizon pillars under design. Sections are intentionally light on dates — Modulatio ships when it's ready, not on a calendar — and heavy on what each direction is *for*.
