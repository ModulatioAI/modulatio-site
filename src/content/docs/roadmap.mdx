...
v0.9.9.0 is the current Beta release — **the WebOS**: the TUI's whole layout rendered in your browser, hooked straight to the same engine seams the terminal already uses. This page consolidates what's shipping next (the **1.0** line) and the long-horizon pillars under design. Sections are intentionally light on dates — Modulatio ships when it's ready, not on a calendar — and heavy on what each direction is *for*.
