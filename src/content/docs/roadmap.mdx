...
v0.8.6 is the current Beta release. This page consolidates what's shipping next and the long-horizon pillars under design. Sections are intentionally light on dates — Modulatio ships when it's ready, not on a calendar — and heavy on what each direction is *for*.
...
The current release is **v0.8.6 — Leader self-remediation + JT generativity** ([release notes](/v0-8-6/)). Two arcs on one principle (the engine binds the hard invariant, prose bends the judgment): the Leader now **remediates fixable concerns in place** — under a typed declare-validate gate, an engine-owned fix window, a HARD-violation withhold that survives delivery, and an access invariant a redo can't widen — instead of punting to a ticket; and the engine **refuses to run a saved job template a job can't fill** (a mechanical-fit gate, fail-closed), **derives a fitting one** through a guided interview, and **skips a drifted cron slot** (with the reason) rather than improvising unsupervised content. It builds on **v0.8.4 — deliverable fidelity**: the engine now verifies the *whole* assembled deliverable against the brief, not just the parts. A declared **`DeliverableSpec`** carries the per-part floor / required structure / title into the run; an engine-extracted **digest + readable twin** give the verifier real eyes; the per-part floor is **bound at produce**; and the engine **generates the framing** (title + TOC) and **normalizes part numbering** — all per-family dispatch, product- and agent-agnostic. See the [v0.8.4 release notes](/v0-8-4/) and the [Deliverable fidelity](/architecture/deliverable-fidelity/) deep-dive. The full release history (v0.7.x conversation-first + key pool, v0.8.x assembler families + metered tier) is in the **Releases** menu.
...
Production-scale efforts (the 200-page-novel class, the multi-quarter software project) become canonical use cases. Modulatio's Phase-1-only constraint goes away, replaced by template-driven multi-phase orchestration where each phase gates between sign-offs.

### Persona / identity continuity

A sibling slice to job templates: keep team identity stable across long-running deployments. Job templates carry the *what* across phases; the persona slice carries the *who*. See [the Beta calibration "Persona / identity continuity" gap section](/v0-1-0-beta/#persona--identity-continuity-across-long-running-deployments) for the user-facing impact.
